    ...
    <el-table ref="withdraw" v-loading="loading" :data="dataList" style="width: 100%">
      ...
      <el-table-column label="用户id" prop="uid" width="80">
        <template slot-scope="scope">
          <span v-html="rechargeUidFormat(scope.row)"/>
        </template>
      </el-table-column>
      ...
      <el-table-column label="金额" prop="amount" width="200">
        <template slot-scope="scope">
          <span v-html="rechargeAmountFormat(scope.row)"/>
        </template>
      </el-table-column>
...
import {collect, recharges, updateScore} from "../../../../api/bigo/bicai";
...
export default {
  ...
  methods: {
    ...
    rechargeUidFormat(row, column) {
      if(row.rechargeCount <= 1){
        return '<font color="red">' + row.uid +'</font>'
      }else if(row.rechargeCount == 2){
        return '<font color="blue">'+ row.uid +'</font>'
      }else{
        return row.uid
      }
    },
    rechargeAmountFormat(row, column) {
      if(row.rechargeCount <= 1){
        return '<font color="red">' + row.amount +'</font>'
      }else if(row.rechargeCount == 2){
        return '<font color="blue">'+ row.amount +'</font>'
      }else{
        return row.amount
      }
    },
