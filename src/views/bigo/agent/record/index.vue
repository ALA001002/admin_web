...
<!--      <el-form-item label="交易哈希" prop="hash">
    ...
    <el-table v-loading="loading" :data="transactionList">
      ...
      <el-table-column label="用户账号" align="center" prop="username" min-width="150"/>
      <el-table-column label="上级ID" align="center" prop="parentUid" />
      <el-table-column label="业务员ID" align="center" prop="salesmanId" />
      <el-table-column label="顶级ID" align="center" prop="topUid" />
      ...
      <el-table-column label="折算价(U)" align="center" prop="convertedPrice" />
      <el-table-column label="行情(U)" align="center" prop="symbolPrice" />
      <el-table-column label="转出地址" align="center" prop="from" min-width="200"/>
      <el-table-column label="目标地址" align="center" prop="to" min-width="200"/>
...
<script>
import { lowerRecordList,lowerExport } from "@/api/bigo/transaction";
...
export default {
  ...
  created() {
    ...
    const parentUid = this.$route.query.uid;
    this.getRecord(coin, parentUid);
    // this.getList();
  },
  methods: {
    getRecord(coin, parentUid) {
      this.queryParams.coin = coin;
      this.queryParams.parentUid = parentUid;
    ...
    getList() {
      this.loading = true;
      lowerRecordList(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有交易记录?', "警告", {
        ...
        return lowerExport(queryParams);
      }).then(response => {
